...
def linregress_1d3d(var1d, var3d):
  ...
  for x in range(nx):
    for y in range(ny):
      ...
      mod2 = np.compress(bad, var3d[:,y,x])
      try:
        reg[y,x], intr[y,x], _, regp[y,x], _ = stats.linregress(mod1, mod2)
      except:
        continue
  return reg, intr, regp

### Calculate correlation
cor, corp = corr_1d3d(oni, sstJJA)

### Calculate regression
reg, intr, regp = linregress_1d3d(oni, sstJJA)
...
def get_pval_regr_slope(x,y,slope,intercept,Neff=None):
    ...
    ### two-tailed, 1-p_val
    p_val= stats.t.sf(np.absolute(t),df=Neff-2)*2
    sf_level= 1-p_val
    if t<0: sf_level*=-1
...
for i in range(nx):
  for j in range(ny):
    y = sstJJA[:,j,i]
    pval_mod[j,i], _ = get_pval_regr_slope(oni,y,reg[j,i],intr[j,i],Neff=Neff[j,i])
...
### color set to white
plot_global(0, lon, lat, reg, regp, clevs,mycolors,'Original p-value of Reg[ONI index, SST]')
plot_global(1, lon, lat, reg, pval_mod, clevs,mycolors,'Modified p-value of Reg[ONI index, SST]')

#plt.tight_layout()
plt.subplots_adjust(bottom=0.2, top=0.9, left=0.1,right=0.9)
plt.savefig('./Figure/8-4_T-test_effective_freedom_regr.png', bbox_inches='tight')
